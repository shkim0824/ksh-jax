...
def custom_init():
    returned_function = jax.nn.initializers.variance_scaling(0.02**2, mode="fan_in", distribution="truncated_normal")
...
class ViTConfig():
    ...
    def __init__(
        ...
        qkv_bias = True,
        encoder_stride = 16,
        adapter_size = 64, # middle dimension of adapter module
    ):
...
class ViTLayer(nn.Module):
    ...
    def setup(self):
        ...
        self.adapter = ViTAdaptFormer(self.config) # AdaptFormer module
        self.adapter_scale = self.param("adapter_scale", jax.nn.initializers.constant(0.1), (1,),) # scale parameter, which scales adapter output. Default=0.1 in paper
    ...
    def __call__(self, hidden_states, deterministic: bool = True, output_attentions: bool = False):
        ...
        hidden_states = self.output(hidden_states, deterministic=deterministic)
        ...
        hidden_states = hidden_states + self.adapter_scale * adapter_output + attention_output # original adaptFormer
...
class ViTAdaptFormer(nn.Module):
    """
    AdaptFormer module for ViT MLP layer
    """
    config: ViTConfig
        
    def setup(self):
        self.layernorm_adapter = nn.LayerNorm(epsilon=self.config.layer_norm_eps)
        self.down = nn.Dense(
            self.config.adapter_size, # adapter_size is bottleneck size, default=64
            kernel_init=jax.nn.initializers.variance_scaling(
                self.config.initializer_range**2, "fan_in", "truncated_normal"
            ),
            use_bias=False
    ...
    def __call__(self, hidden_states):
        hidden_states = self.down(self.layernorm_adapter(hidden_states)) # scaling down
        hidden_states = self.activation(hidden_states) # activation
        hidden_states = self.up(hidden_states) # scaling up
